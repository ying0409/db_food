...
if (isset($_POST['s_name'])) {
	...
	// echo "<h3>$style</h3>";
	// echo "<h3>$time</h3>";
	if($price!=0&&$style!=null&&$day!=0&&$time!=0)$sort_sql = "SELECT DISTINCT(s_id),s_name,photo,avestar FROM shop natural join time WHERE aveprice>='$price'and aveprice<'$price'+50 and style='$style' and day='$day' and  '$time'>op_hr and '$time'+2<cl_hr ORDER BY avestar desc, aveprice asc";
	//echo "<h3>$sort_sql</h3>";
	...
	if ($result->num_rows > 0){
		...
		while($row = mysqli_fetch_array ( $result, MYSQLI_ASSOC ) ) {
			if($count%2==0)echo "<tr>";
				echo "<td><a href = 'customer_shop.php? my_id=$my_id&num=$row[s_id]'><b>$row[s_name]</b></a><br>";
				switch ($row[avestar]) {
					case 0:
						echo "☆☆☆☆☆";
						break;
